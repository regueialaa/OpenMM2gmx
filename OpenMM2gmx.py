...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="xmlfile",
        required=True,
        help="Input OpenMM coordinates/system information XML file from the production run ",
    )
    parser.add_argument(
        "--traj",
        dest="trajfile",
    ...
    parser.add_argument(
        ...
        dest="sim_name",
        required=True,
        help="Name of the simulation_replica if needed",
    )
    parser.add_argument(
        "--stride",
        dest="stride",
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "0 = full corrected system trajectory (Protein + non-polymer stuff),"
...
def generate_gro_top_files(top_file, xml_file, sim_name, output_dir):
    ...
    logger.info("- Detected box vectors: %s", structure.box)
    structure.save(GRO_OUTPUT, format="gro", overwrite=True)
    structure.save(TOP_OUTPUT, format="gromacs", overwrite=True)
    with open(
        f"{output_dir}/log_files/{sim_name}_generation_gro_top.log", "w"
    ) as log_file:
        log_file.write(f"Detected box vectors:\n{structure.box}\n")
        log_file.write(f"Generated files: {GRO_OUTPUT}, {TOP_OUTPUT}\n")

    logger.info(
        f"- Conversion completed successfully, Generated files: {GRO_OUTPUT}, {TOP_OUTPUT}"
    )
...
def make_index_file(GRO_OUTPUT, center_res, sim_name, output_dir):
    ...
    with open(
        f"{output_dir}/log_files/{sim_name}_generation_index.log", "w"
    ) as log_file:
        log_file.write(result.stdout)
        log_file.write(result.stderr)
...
def traj_correction(
    ...
    centered_xtc_full_complex = (
        f"{output_dir}/{sim_name}_trj_no_jump_centered_full_complex.xtc"
    )
    centered_xtc_only_protein = (
        f"{output_dir}/{sim_name}_trj_no_jump_centered_only_protein.xtc"
    )
